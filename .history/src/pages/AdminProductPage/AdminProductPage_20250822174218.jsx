...
const AdminProductPage = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/products/stats/summary`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
        ...
        <Space size="middle">
          ...
          >
            Sửa
             ...
             description="Hành động này không thể hoàn tác. Sản phẩm sẽ bị xóa vĩnh viễn khỏi database."
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      <Modal
        ...
        onCancel={handleCloseAddProductForm}
        footer={null}
        width={800}
        centered
        destroyOnClose
