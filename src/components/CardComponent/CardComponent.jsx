...
const CardComponent = ({ product }) => {
  const { _id, id, name, image, mainImage, price, rating, sold, discount } = product || {}
    ...
    <div style={{ position: 'relative' }}>
      <Link to={`/product-details/${productId}`} style={{ textDecoration: 'none' }}>
        <WrapperCardStyle
          ...
          cover={
            <div style={{ position: 'relative' }}>
              <img alt={name} src={displayImage} style={{ height: '200px', objectFit: 'cover', width: '100%' }}/>
              {/* Icon giỏ hàng nằm trong ảnh */}
              <Button
                type="primary"
          ...
          >
              <WrapperContent>
                  <StyleNameProduct>{name || 'Tên sản phẩm'}</StyleNameProduct>
                  <WrapperReportText>
